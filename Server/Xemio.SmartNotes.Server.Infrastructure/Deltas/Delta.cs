...
namespace Xemio.SmartNotes.Server.Infrastructure.Deltas
{
    ...
    public class Delta<TEntityType> : DynamicObject where TEntityType : class
    {
        ...
        public bool TrySetPropertyValue(string name, object value)
        {
            ...
            var isJArray = typeof (IEnumerable).IsAssignableFrom(cacheHit.Property.PropertyType) && value is JArray;
            ...
            if (isJArray) value = ((JArray) value).ToObject(cacheHit.Property.PropertyType);
        ...
        public bool HasPropertyChanged<TPropertyType>(Expression<Func<TEntityType, TPropertyType>> expression)
        {
            string propertyName = expression.GetMemberInfo().Name;
            return this._changedProperties.Contains(propertyName);
        }

        public TPropertyType GetPropertyValue<TPropertyType>(Expression<Func<TEntityType, TPropertyType>> expression)
        {
            string propertyName = expression.GetMemberInfo().Name;
            return (TPropertyType)this._propertiesThatExist[propertyName].GetValue(this._entity);
        }
